...
for kk in range(0,10,1):
    ...
    for ii in range(0, map1.shape[0], slicesize):
        for jj in range(0, map1.shape[1], slicesize):
            ...
            xa, ya = map2.shape
            #reshape the input
            image_2d = map2.reshape(xa*ya, 1)
            #set up and fitting the clustering system
            ...
            #reorganize the value based on the cluster centerswith respective cluster labels.
            im3=cluster_centers[cluster_labels].reshape(xa, ya)
            print(im3.shape)
            #im3=np.max(im3,axis=1)
            #print(im3.shape)
            im=im3
